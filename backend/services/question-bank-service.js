class QuestionBankService {
    ...
    try {
      ...
      const result = await this.dbService.get(
        `SELECT COUNT(*) as count FROM cards c
         ...
         WHERE p.questionbank_id = $1 AND c.card_type = 'question'`,
        [questionbankId]
      );
      
      // In PostgreSQL, COUNT(*) returns a string, so convert to number
  ...
  async getQuestionBankById(questionbankId, contentRetriever, client) {
    // Use the provided client if available, otherwise use the dbService
    const queryExecutor = client || this.dbService;
    
    try {
      // Get the main question bank record
      const questionBank = await queryExecutor.get(
      ...
      const pages = await queryExecutor.all(
      ...
      const processedPages = await Promise.all(
        pages.map(async (page) => {
          ...
          const cards = await queryExecutor.all(
          ...
          const processedCards = await Promise.all(
            cards.map(async (card) => {
              const contents = await contentRetriever.getCardContents(card.id, card.card_type, client);
